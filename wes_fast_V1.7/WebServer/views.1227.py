...
def result(request):
	...
	DB_data = models.Test.objects(Q(bus_code__exact = 'xxdfgwergsd234asdf'))
	filter_id=[]
	result_dict1 = None
	result_dict2 = None
	result_dict3 = None
	...
	if request.method == 'POST':
		Search_Flag = 1
		DB_data = models.Test.objects.order_by('bus_code')
		...
		# models.Test.objects.filter(bus_code__exact='7E6756').update(clinical_info=sam_type1)
		...
		#es=DB_data.get(id='5c1076f917b2bf845606e201')
		#print es.bus_code
		for m in DB_data:
			oid=m.id
			a=ObjectId(oid)
		...
		for i in filter_id:
			tmp=DB_data.filter(id=i)
			tmp.remove()
	#print "I:",filter_id
	print "I:",len(filter_id)
	print 'C2:',DB_data.count()
...
def cartgram(request):
	...
	if request.method == 'POST':
		...
		DB_Data_All=models.Test.objects.all()
		dataList1_cache = ''
		dataList2_cache = ''
